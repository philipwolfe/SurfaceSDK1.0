...
namespace Cloth.UI
{
    ...
    public abstract class UIElement : DrawableGameComponent
    {
        ...
        // Determines if a UIElement responds to contacts.
        ...
        private SpriteBlendMode spriteBlendMode = SpriteBlendMode.None;
            ...
            : base(game)
        {
            ...
            if (parent != null)
            {
                ...
                spriteBlendMode = parent.SpriteBlendMode;
                spriteSortMode = parent.spriteSortMode;
                SaveStateMode = parent.SaveStateMode;
        ...
        /// <summary>
        /// Read-write property that determines if this UIElement should respond to contacts.
        /// </summary>
        public bool Active
        {
            get { return active; }
            set
            {
                ...
                if (active != value)
                {
                    ...
                    if (active == false)
                    {
                        // Release any captured contacts.
                        // StateMachine.ReleaseCapturedContacts();
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Read-write property that gets or set the SpriteBlend used to initialize this
        ...
        /// <summary>
        /// Read-write property that gets or set the SaveStateMode used to initialize this
        /// UIElement's spriteBatch.
        /// </summary>
        ...
        public string TextureSourceFile
        {
            ...
            set
            {
                ...
                {
                    Texture = Texture2D.FromFile(GraphicsDevice, value);
                }
            }
        }
           ...
           if (textureSourceFile != null)
            {
                if (AutoScaleTexture)
                {
                      texture = Texture2D.FromFile(Game.GraphicsDevice, textureSourceFile, width, height);
                }
                else
                {
                      texture = Texture2D.FromFile(Game.GraphicsDevice, textureSourceFile);
                }
            }
        ...
        public override void Draw(GameTime gameTime)
        {
            if (parent == null)
            {
                ...
                spriteBatch.Begin(spriteBlendMode, spriteSortMode, SaveStateMode, screenTransform);
            }
        ...
        /// <list>
        /// <item>An uncaptured contact over the element.</item>
        /// <item>A previously captured contact still over the element.</item>
        /// <item>A previously captured contact no longer over the element.</item>
        /// </list>
        /// Determining what the HitTestDetails should be in this last case may require
        /// some thought.
        /// </para>
        /// </remarks>
        /// <param name="contact">The contact for which hit test details should be supplied.</param>
        /// <param name="captured">boolean indicating if the contact was previously captured.</param>
        /// <returns></returns>
        public virtual IHitTestDetails HitTestDetails(Contact contact, bool captured)
        {
            return null;
        }

        /// <summary>
        /// Returns true if the contact is contained within this element.
        /// </summary>
        /// <param name="contact">The contact to hit test.</param>
        /// <param name="captured">Boolean indicating if the contact was previously captured.</param>
        /// <returns></returns>
        public virtual bool HitTest(Contact contact, bool captured)
        {
            // Controls must be Active and Visible in order to respond to contacts.
            if (!Active || !Visible) { return false; }

            Vector2 contactVector = Vector2.Transform(new Vector2(contact.CenterX, contact.CenterY), screenTransform);

            return DrawingRectangle.Contains(Convert.ToInt32(contactVector.X),
                                             Convert.ToInt32(contactVector.Y));
        }
        ...
        /// <param name="captured">Boolean indicating if the contact was previously captured.</param>
        /// <returns>The UIElement hit by a contact or null if no hit is found.</returns>
        public UIElement HitTesting(Contact contact, bool captured)
        {
            // Test each of this elements children first from front to back.
            // Maintain children in LayerDepth order.
            foreach (UIElement child in children)
            {
                UIElement hit = child.HitTesting(contact, captured);
                if (hit != null)
                {
                    return hit;
                }
        ...
        /// <summary>
        /// Used by the UIController to determine what UIElement is being touched by a given contact.
        /// </summary>
        ...
        public virtual void HitTestCallback(ReadOnlyHitTestResultCollection uncapturedContacts,
                                     ReadOnlyHitTestResultCollection capturedContacts)
        {
            HitTestCapturedContacts(capturedContacts);
            HitTestUncapturedContacts(uncapturedContacts);
        }
        ...
        /// <summary>
        /// Performs hit testing on all uncaptured contacts and updates the hit test information.
        /// </summary>
        /// <param name="uncapturedContacts">Read-only collection of uncaptured contacts to test.</param>
        public virtual void HitTestUncapturedContacts(ReadOnlyHitTestResultCollection uncapturedContacts)
        {
            // Hit test and assign all new contacts.
            foreach (HitTestResult result in uncapturedContacts)
            {
                // Make sure that contact hasn't been assigned already.
                if (result.StateMachine != null) { continue; }

                // Hit test the contact to determine which object it is touching
                UIElement touched = HitTesting(result.Contact, false);
                if (touched != null)
                {
                    IHitTestDetails details = touched.HitTestDetails(result.Contact, false);
                    result.SetUncapturedHitTestInformation(touched.StateMachine, details);
                }
                else
                {
                    // This contact has not been captured.
